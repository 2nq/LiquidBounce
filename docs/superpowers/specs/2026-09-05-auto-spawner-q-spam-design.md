...
The controller uses four states: idle, waiting for the menu, spamming, and halted. Enabling starts a new generation, sends the command, and schedules the transition to spamming. Once spamming, each player tick attempts one throw action on slot 10. Generation checks prevent stale scheduled work from surviving a disable/enable transition.
...
- disabling and the generation guard stop further actions;
